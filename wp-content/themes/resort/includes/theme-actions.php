...
- Customise the testimonial template for the "Testimonials by WooThemes" plugin.
		...
		if ( is_array($woo_options) ) {
			...
			if ( isset( $woo_options['woo_typography'] ) && $woo_options['woo_typography'] == 'true' ) {
				...
				if ( isset( $woo_options['woo_font_nav_home'] ) && $woo_options['woo_font_nav_home'] )
					$output .= '.home .nav a { '.woo_generate_font_css($woo_options['woo_font_nav_home'], '1.4').' }' . "\n";	

				if ( isset( $woo_options['woo_font_page_title'] ) && $woo_options['woo_font_page_title'] )
					$output .= '.page header h1 { '.woo_generate_font_css($woo_options[ 'woo_font_page_title' ], '1.4').' }' . "\n";
			
				if ( isset( $woo_options['woo_font_post_title'] ) && $woo_options['woo_font_post_title'] )
					$output .= '.post header h1 { '.woo_generate_font_css($woo_options[ 'woo_font_post_title' ], '1.4').' }' . "\n";	

				if ( isset( $woo_options['woo_font_post_title_link'] ) && $woo_options['woo_font_post_title_link'] )
					$output .= '.post header h1 a:link, .post header h1 a:visited { '.woo_generate_font_css($woo_options[ 'woo_font_post_title' ]).' }' . "\n";
...
if ( ! function_exists( 'woo_layout_body_class' ) ) {
	function woo_layout_body_class( $classes ) {
		...
		$settings = woo_get_dynamic_values( array( 'featured' => 'true', 'connect' => 'false', 'connect_footer' => 'true', 'homepage_enable_intro_message' => 'false' ) );
		...
		if ( is_home() && isset( $settings['homepage_enable_intro_message'] ) && $settings['homepage_enable_intro_message'] == 'true' ) {
			$classes[] = 'has-intro-message';
		}		

		// Add body class if Connect Footer is enabled
		if ( isset( $settings['connect_footer'] ) && $settings['connect_footer'] == 'true' ) {
			$classes[] = 'has-connect-footer';
		}
