...
function woocommerce_output_related_products() {
	...
	if ( isset( $woo_options['woocommerce_related_products'] ) && 'true' == $woo_options['woocommerce_related_products'] ) {	
...
if ( ! function_exists( 'woocommerce_theme_before_content' ) ) {
	function woocommerce_theme_before_content() {
		...
		if ( is_product() ) { $columns = 'woocommerce-columns-4'; }
...
add_action( 'woo_nav_after', 'woo_header_components', 30 );

function woo_header_components() {

	global $woo_options;
	global $woocommerce;

	echo '<div id="header-components">';

	if ( is_woocommerce_activated() && isset( $woo_options['woocommerce_header_cart_link'] ) && 'true' == $woo_options['woocommerce_header_cart_link'] ) { ?>
		<ul class="nav cart">
			<li><a class="cart-contents" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'woothemes' ); ?>"><span class="items"><?php echo sprintf( _n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes' ), $woocommerce->cart->cart_contents_count );?></span></a></li>
		</ul>
	<?php
	}

	echo '</div><!-- /#header-components -->';
...
if ( ! function_exists( 'woocommerce_header_add_to_cart_fragment' ) ) {
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		ob_start();
?>
		<a class="cart-contents" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'woothemes' ); ?>"><span class="items"><?php echo sprintf( _n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes' ), $woocommerce->cart->cart_contents_count );?></span></a>
<?php
